class PostsController < ApplicationController
	...
	def index
		...
		@posts = if params[:term] 
					Post.where('title LIKE :title1 OR body LIKE :body1',{:title1 => "%#{params[:term]}%".downcase, :body1 => "%#{params[:term]}%".downcase}).all.order("updated_at DESC").paginate(:page=> params[:page], :per_page=> 5)
				else
